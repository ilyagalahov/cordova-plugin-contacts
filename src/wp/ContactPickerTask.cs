    ...
    public class ContactPickerTask
    {
        ...
        public void Show()
        {
            Deployment.Current.Dispatcher.BeginInvoke(() =>
            {
                ...
                if (root != null)
                    root.Navigated += NavigationService_Navigated;

                string baseUrl = "/";
                // dummy parameter is used to always open a fresh version
                if (root != null)
                    root.Navigate(new Uri(baseUrl + "Plugins/org.apache.cordova.contacts/ContactPicker.xaml?dummy=" + Guid.NewGuid().ToString(), UriKind.Relative));
            });
        }
        ...
        {
            if (!(e.Content is ContactPicker)) return;

            var phoneApplicationFrame = Application.Current.RootVisual as PhoneApplicationFrame;
            if (phoneApplicationFrame != null)
                phoneApplicationFrame.Navigated -= NavigationService_Navigated;

            ContactPicker contactPicker = (ContactPicker)e.Content;

            if (contactPicker != null)
            {
                contactPicker.Completed += Completed;
            }
            else if (Completed != null)
            {
                Completed(this, new PickResult(TaskResult.Cancel));
            }
        }
